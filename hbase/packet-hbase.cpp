...
bool dissect_rpcheader(tvbuff_t *tvb, guint* offset, proto_tree *tree, bool* bRequest, int *callId, string& rpcMethodName)
{
    ...
    *bRequest = true;
    ...
    map<string, Handles*>::iterator itRequest = g_mapHandles.find( "RequestHeader" );
    ...
    if (reqMessage->ParseFromArray(rpcHeaderBuf,rpcHeaderLen))
    {
		...
		field = reqMessage->GetDescriptor()->FindFieldByName("method_name");
        rpcMethodName = reflection->GetString(*reqMessage, field);
		if ("" != rpcMethodName)
		{
            field = reqMessage->GetDescriptor()->FindFieldByName("call_id");
            *callId = reflection->GetUInt32(*reqMessage, field);

		    dissect_protobuf_message(reqMessage, tvb, offset, tree, string("RequestHeader"), true);
            *bRequest = true;
    ...
    // maybe response
    map<string, Handles*>::iterator itResponse = g_mapHandles.find( "ResponseHeader" );
    ...
    {
        dissect_protobuf_message(responseMessage, tvb, offset, tree, string("ResponseHeader"), true);
        *bRequest = false;
            
        // get callid 
        const FieldDescriptor *field = NULL;
        field = responseMessage->GetDescriptor()->FindFieldByName("call_id");
...
bool dissect_hadoop_rpc(tvbuff_t *tvb, guint* offset, proto_tree *hadoop_tree, packet_info *pinfo)
{
    ...
    dissect_rpcheader(tvb, offset, hadoop_tree, &bRequst, &callId, rpcMethodName);
    if (bRequst)
    {
...
static int dissect_hadoop(tvbuff_t *tvb, packet_info *pinfo, proto_tree *tree, void* data)
{
    ...
    col_set_str(pinfo->cinfo, COL_PROTOCOL, "HBASE");
    ...
    if (tree) 
    {
        ...
        /* first setup packet starts with "hbase" */
...
void proto_register_hadoop(void)
{
    ...
    string pbFilePath = get_persconffile_path("hbase", false);
    ...
    new_register_dissector("hbase", dissect_hadoop, proto_hadoop);
}
...
void proto_reg_handoff_hadoop(void)
{
    ...
    if (!initialized) 
	{
        hadoop_handle = find_dissector("hbase");
